document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("symbolsLabel").textContent = chrome.i18n.getMessage("specialSymbols");
  document.getElementById("passwordCountLabel").textContent = chrome.i18n.getMessage("passwordCount") + ":";
  document.getElementById("generate").textContent = chrome.i18n.getMessage("generateButton");

  const generateBtn = document.getElementById("generate");
  generateBtn.addEventListener("click", generatePasswords);

  document.querySelectorAll(".input-group button.decrement").forEach((button) => {
  ...
  document.querySelectorAll(".input-group button.increment").forEach((button) => {
    button.addEventListener("click", (event) => {
      const input = event.target.parentNode.querySelector("input");
...
function generatePasswords() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (symbols) {
      const symbol = symbolCharset.charAt(Math.floor(Math.random() * symbolCharset.length));
      const insertPosition = Math.floor(Math.random() * length);
      password = password.slice(0, insertPosition) + symbol + password.slice(insertPosition);
    }
